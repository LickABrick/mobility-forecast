...
P1 adds the real config-entry setup/unload hooks and forwards only the sensor platform. Setup creates a distinct runtime per entry before forwarding. Until the separately reviewed Store and source-adapter checkpoints replace the pending boundaries, coordinator refresh and diagnostics fail closed rather than fabricating state; the passive sensor therefore remains unavailable. Successful platform unload clears runtime data, while failed unload retains it for entities that remain loaded. The lifecycle creates no task, timer, update interval, provider call or I/O path.
